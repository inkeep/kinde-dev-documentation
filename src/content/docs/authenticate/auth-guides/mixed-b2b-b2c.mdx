...
If you need to enable both SSO (such as SAML or social sign-in) and email authentication for a single customer organization, you can configure multiple authentication methods simultaneously.

1. In Kinde, go to **Settings > Authentication** and ensure both authentication methods are enabled:
   - Enable your desired SSO connection (e.g., SAML, Microsoft Entra ID, Google)
   - Enable email authentication (password or passwordless)
2. Navigate to the relevant organization where you want both methods available.
3. Go to the organization's **Authentication** settings.
4. Switch on both the SSO connection and the email authentication method for that organization.
5. Save your changes.

With both methods enabled, users in that organization will see options for both SSO and email authentication on the sign-in screen, allowing them to choose their preferred method.
